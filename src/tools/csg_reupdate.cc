...
void CsgREupdate::BeginEvaluate(Topology *top, Topology *top_atom){
  ...
  if (_PERMITTIVITY && !_gentable){
      _nlamda =_nlamda +1;
   //   Table *_UCoul =  new Table();
   //   _UCoul->Load("Coul_pot.new");
   //   int globalcounter = 0;
 }
...
void CsgREupdate::Run(){
  ...
  if( !_gentable )
    ...
    for (list<Property*>::iterator iter = _nonbonded.begin();
      ...
      PotentialInfo *i = new PotentialInfo(_potentials.size(),false,_nlamda,_param_in_ext,*iter,_gentable);
...
void CsgREupdate::REUpdateLamda() {
 ...
 _DECOUPLE = _options.get("cg.inverse.decouple").as<bool>();
 if(_DECOUPLE){
  for(int row = 0; row < _nlamda; row++){
    //for(int col=0; col <_nlamda; col ++)
//    for(int col = 0; col<_nlamda; col++){
     // cout<<col<<"\t"<<row<<"\t"<<_HS(row,col)<<endl;
      HS_(row,row) = _HS(row,row);
      HS2_(row, row) = _HS(row,row);
      }
 } 
 else {
  for(int row = 0; row < _nlamda; row++)
  ...
  if(_PRESSURE){
   ...
   // cout<< dlamdaC(_nlamda-1)<<"Here"<<dlmada(_nlamda-1)<<endl;
...
void CsgREupdateWorker::EvalConfiguration(Topology *conf, Topology *conf_atom){
   ...
   */
   ...
   if (_PERMITTIVITY){
      ...
      std::ifstream ofs2("task.new", std::ofstream::in);
      double task;
      ofs2 >> task;
      ofs2.close();			     
      ...
      _UCoul->Load("Coul_pot.new");
       ...
       int step;
       step = conf->getStep();
       // double jump = _UCoul->x(2) - _UCoul->x(1);
       int i = _nframes;
       while( i <  _UCoul->size()){
          if ( step == _UCoul->x(i)){
	     _dUFrame(_nlamda-1) = _UCoul->y(i)*num;
//	     cout<<num<<"\t #Step \t _UCoul :\t"<< _UCoul->x(i)<<"\t"<<_UCoul->y(i)<<"_nframe and step\t:"<<_nframes<<"\t"<<step<<"\ti:\t"<<i<<endl;
	     break;
	    }
	    i++;
       }
